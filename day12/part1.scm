...
(define (num-soln pat seq)
  ...
  (case (car pat)
    ((#\.) (if (is-bind seq) 
	     (if (is-zero-bind seq) (num-soln (cdr pat) (cdr seq)) 0)
	     (num-soln (cdr pat) seq)))
    ((#\#) (cond ((null? seq) 0)
		 ((is-bind seq) 
		   (if (is-zero-bind seq) 0
		     (num-soln (cdr pat) (dec-bind seq))))
	    	 (else (num-soln (cdr pat) (start-bind seq)))))
    ((#\?) (+ (num-soln (cons #\. (cdr pat)) seq)
	      (num-soln (cons #\# (cdr pat)) seq))))))
